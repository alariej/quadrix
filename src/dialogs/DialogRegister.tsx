...
export default class DialogRegister extends RX.Component<DialogRegisterProps, DialogRegisterState> {
	...
	private onCompleteRecaptcha = (captchaToken: string) => {
		if (['error', 'expired'].includes(captchaToken)) {
			return;
		}
		...
		return new Promise(resolve => {
			const text = (
				<RX.Text style={styles.textDialog}>
					<RX.Text>{confirmationEmail[this.language]}</RX.Text>
					<RX.Text style={{ fontWeight: 'bold' }}>{this.emailAddress}</RX.Text>
					<RX.Text>{firstClickLink[this.language]}</RX.Text>
				</RX.Text>
			);
			...
			const content = (
				...
				>
					<RX.View style={styles.emailInputMessage}>
						<RX.Text style={styles.textDialog}>{serverRequiresEmail[this.language]}</RX.Text>
					</RX.View>
