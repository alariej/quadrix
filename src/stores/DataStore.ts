...
class DataStore extends StoreBase {
    ...
    private setDirectRoomList(syncData: SyncResponse_) {
        ...
        if (!syncData.account_data) { return; }
    ...
    private redactMessage(event: MessageEvent_, roomIndex: number) {
        ...
        if (messageIndex > -1) {
            ...
            this.roomSummaryList[roomIndex].lastRedactedEventId = this.roomSummaryList[roomIndex].timelineEvents[messageIndex].event_id;
    ...
    private setRoomInfoFromEvents(events: MessageEvent_[], roomIndex: number): RoomEventTriggers {
        ...
        if (!events || events.length === 0) { return roomEventTriggers }
    ...
    private setPowerLevel(event: MessageEvent_, roomIndex: number) {
        ...
        Object.keys(event.content.users!).map(user => {
            ...
            this.roomSummaryList[roomIndex].members[user] = { ...this.roomSummaryList[roomIndex].members[user], ...member }
        });
    }
    ...
    private setRoomInfoFromSummary(summary: RoomSummary_, roomIndex: number) {
        ...
        summary['m.joined_member_count'] ? this.roomSummaryList[roomIndex].joinMembersCount = summary['m.joined_member_count'] : 0;
        summary['m.invited_member_count'] ? this.roomSummaryList[roomIndex].inviteMembersCount = summary['m.invited_member_count'] : 0;
    ...
    private setRoomType(roomId: string, roomIndex: number) {
        ...
        if (this.roomSummaryList[roomIndex].type) { return }
    ...
    private setRoomActive(roomIndex: number): boolean {
        ...
        if (this.roomSummaryList[roomIndex].type === 'community') {
            ...
            this.roomSummaryList[roomIndex].active = (
                this.roomSummaryList[roomIndex].joinMembersCount! > 1
            );
        }
    ...
    private updateTimeLine(timeline: RoomTimeline_, roomIndex: number) {
        ...
        */
        ...
        if (timeline.limited && timeline.prev_batch.slice(0, 1) !== 's') {
            ...
            this.roomSummaryList[roomIndex].timelineEvents = this.roomSummaryList[roomIndex].timelineEvents.concat(timeline.events);
        }
    }
    ...
    public updateRoomSummaryList(syncData: SyncResponse_) {
        ...
        for (const roomId in syncData.rooms.join) {
            ...
            if (roomIndex > -1 && this.roomSummaryList[roomIndex].phase === 'join' && this.roomSummaryList[roomIndex].type) {

                roomEventTriggers = this.updateJoinRoom(syncData.rooms.join[roomId], roomIndex);

            } else if (roomIndex > -1 && this.roomSummaryList[roomIndex].phase === 'join' && !this.roomSummaryList[roomIndex].type) {
    ...
    private updateReadReceipts(roomObj: RoomData_, roomIndex: number): boolean {
        ...
        if (!roomObj.ephemeral || !roomObj.ephemeral.events) { return false; }
                ...
                for (const eventId in content) {
                    for (const userId in content[eventId]['m.read']) {
                        ...
                        this.lastSeenTime[userId] = Math.max(
                            this.lastSeenTime[userId] || 0, newReceipts[userId].timestamp,
                        );
                    }
                }
            });

        this.roomSummaryList[roomIndex].readReceipts = { ...this.roomSummaryList[roomIndex].readReceipts, ...newReceipts };
    ...
    public getUnreadTotal(roomId: string) {
        ...
        this.roomSummaryList
            .filter(roomSummary => (
                roomSummary.id !== roomId
            ))
            .map(roomSummary => {
                if (roomSummary.unreadCount === undefined) { return; }
    ...
    public getReadMarker(roomId: string): number {
        ...
        if (Object.keys(this.roomSummaryList[roomIndex].readReceipts!).length < this.roomSummaryList[roomIndex].joinMembersCount!) {
            return 0;
        }
        ...
        for (const userId in this.roomSummaryList[roomIndex].readReceipts) {
            if (userId !== ApiClient.credentials.userIdFull) {
                if (oldestMarker === 0) {
                    ...
                    oldestMarker = Math.min(this.roomSummaryList[roomIndex].readReceipts![userId].timestamp, oldestMarker);
                }
            }
        }
