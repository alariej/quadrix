...
export default class DialogNewDirectConversation extends RX.Component<
	DialogNewDirectConversationProps,
	DialogNewDirectConversationState
> {
	private userId = '';
	...
	public componentDidMount(): void {
		...
		if (users.length > 0) {
			...
			const infoTile = (
				<RX.View style={styles.infoTile}>
					<RX.Text style={styles.textDialog}>{searchInstruction[this.language]}</RX.Text>
				</RX.View>
			);
		...
		ApiClient.searchUser(this.userId)
			.then(response => {
				if (response.limited) {
					const infoTile = (
						<RX.View style={styles.infoTile}>
							<RX.Text style={styles.textDialog}>{tooManySearchResults[this.language]}</RX.Text>
						</RX.View>
					);
					...
					const infoTile = (
						<RX.View style={styles.infoTile}>
							<RX.Text style={styles.textDialog}>{noSearchResults[this.language]}</RX.Text>
						</RX.View>
					);
	...
	private createNewDirect = () => {
		...
		if (this.userId === ApiClient.credentials.userIdFull) {
			const text = <RX.Text style={styles.textDialog}>{warningNoSelfDirect[this.language]}</RX.Text>;

			RX.Modal.show(
				<DialogContainer
					content={text}
					modalId={'warning_noself'}
				/>,
				'warning_noself'
			);
		...
		const existingDirectRooms = DataStore.getSortedRoomList()
			.filter(room => room.contactId === this.userId)
				...
				.catch(_error => {
					...
					const text = (
						<RX.Text style={styles.textDialog}>
							...
							<RX.Text style={{ fontWeight: 'bold' }}>{' ' + this.userId + ' '}</RX.Text>
					...
					const modalDialog = (
						<DialogContainer
							...
							onCancel={() =>
								RX.Modal.show(
									<DialogNewDirectConversation showRoom={this.props.showRoom} />,
									'createdirectdialog'
								)
							}
						/>
	...
	public render(): JSX.Element | null {
		...
		const userList = (
			<RX.View
				style={[styles.userListView, { maxHeight: (TILE_HEIGHT + 1) * (this.state.userTiles?.length || 0) }]}
			>
				<RX.ScrollView
		...
		return <KeyboardAwareView content={content} />;
	}
}
