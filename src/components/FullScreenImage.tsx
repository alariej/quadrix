...
import {
	OPAQUE_DARK_BACKGROUND,
	TRANSPARENT_BACKGROUND,
	SPACING,
	BUTTON_ROUND_WIDTH,
	ICON_FULLSCREEN_FILL,
} from '../ui';
...
export default class FullScreenImage extends RX.Component<FullScreenImageProps, FullScreenImageState> {
	...
	private onPan = (state: RX.Types.PanGestureState) => {
		if (this.zoomedWidth === this.imageWidth) {
			if (!this.screenOrientation || this.screenOrientation === 'portrait') {
				...
				if (this.currentIndex === 0 && swipeR) {
					return;
				}
				if (this.currentIndex === this.imageTimeline.length - 1 && swipeL) {
					return;
				}
				...
				if (this.screenOrientation === 'landscapeR') {
					if (this.currentIndex === 0 && swipeU) {
						return;
					}
					if (this.currentIndex === this.imageTimeline.length - 1 && swipeD) {
						return;
					}
				} else {
					if (this.currentIndex === 0 && swipeD) {
						return;
					}
					if (this.currentIndex === this.imageTimeline.length - 1 && swipeU) {
						return;
					}
				}
	...
	private fadeImage = (inc: number, direction = 1, isLandscape = false) => {
		...
		if (this.currentIndex + inc >= this.imageTimeline.length || this.currentIndex + inc < 0) {
			return;
		}

		if (this.imageTimeline.length - this.currentIndex < 5) {
			if (this.timelineLimited) {
	...
	private slideImage = (inc: number, direction: number, isLandscape: boolean) => {
		...
		this.animatedStyle = RX.Styles.createAnimatedViewStyle({
			...
			transform: isLandscape
				? [{ translateY: this.animatedTransform }]
				: [{ translateX: this.animatedTransform }],
		});
