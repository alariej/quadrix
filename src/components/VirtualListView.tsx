...
export class VirtualListView<ItemInfo extends VirtualListViewItemInfo> extends RX.Component<
	VirtualListViewProps<ItemInfo>,
	VirtualListViewState
> {
	...
	constructor(props: VirtualListViewProps<ItemInfo>) {
		...
		this.state = {
			lastFocusedItemKey: _.some(
				props.itemList,
				(item: { key: string | undefined }) => item.key === props.initialSelectedKey
			)
				? props.initialSelectedKey
				: undefined,
			selectedItemKey: _.some(
				props.itemList,
				(item: { key: string | undefined }) => item.key === props.initialSelectedKey
			)
				? props.initialSelectedKey
	...
	UNSAFE_componentWillUpdate(nextProps: VirtualListViewProps<ItemInfo>, nextState: VirtualListViewState) {
		...
		if (
			nextState.lastFocusedItemKey &&
			!_.some(
				nextProps.itemList,
				(item: { key: string | undefined }) => item.key === nextState.lastFocusedItemKey
			)
		) {
			updateState = true;
		...
		if (
			nextState.selectedItemKey &&
			!_.some(nextProps.itemList, (item: { key: string | undefined }) => item.key === nextState.selectedItemKey)
		) {
			updateState = true;
	...
	private _updateStateFromProps(props: VirtualListViewProps<ItemInfo>, initialBuild: boolean) {
		if (props.logInfo) {
			props.logInfo(
				'Rebuilding VirtualListView State - initial: ' + initialBuild + ', items: ' + props.itemList.length
			);
		}
	...
	private _handleItemListChange(props: VirtualListViewProps<ItemInfo>) {
		...
		for (const item of oldItems) {
			...
			if (!newItemMap.has(item.key)) {
				...
				if (item.key === this.state.lastFocusedItemKey) {
					if (
						!this._focusSubsequentItem(FocusDirection.Down, false, false) &&
		...
		for (const item of props.itemList) {
			...
			if (yPosition <= renderBlockTopLimit) {
				...
				if (yPosition - itemHeight < renderBlockBottomLimit) {
					...
					if (this._activeCells.has(item.key)) {
						this._setCellTopAndVisibility(
							item.key,
							this._shouldShowItem(item, props),
							yPosition - itemHeight,
							!!props.animateChanges
						);
					} else {
						this._allocateCell(
							item.key,
		...
		this._heightOfRenderBlock = this._calcHeightOfItems(
			props,
			this._itemsAboveRenderBlock,
			this._itemsAboveRenderBlock + this._itemsInRenderBlock - 1
		);
		this._heightBelowRenderBlock = this._calcHeightOfItems(
			props,
			this._itemsAboveRenderBlock + this._itemsInRenderBlock,
		...
		if (this._containerHeight === 0) {
			this._containerHeight =
				this._heightAboveRenderBlock + this._heightOfRenderBlock + this._heightBelowRenderBlock;
	...
	private _onLayoutItem = (itemKey: string, newHeight: number) => {
		...
		if (!item.measureHeight) {
			...
			if (this.props.logInfo) {
				this.props.logInfo(
					'Item ' +
						itemKey +
						' listed as known height (' +
						oldHeight +
						'), but got an itemOnLayout anyway? (Reported Height: ' +
						newHeight +
						')'
				);
			}
		...
		if (
			itemIndex < this._itemsAboveRenderBlock ||
			itemIndex >= this._itemsAboveRenderBlock + this._itemsInRenderBlock
		) {
		...
		if (oldHeight !== newHeight) {
			if (this.props.logInfo) {
				this.props.logInfo(
					'onLayout: Item Height Changed: ' + itemKey + ' - Old: ' + oldHeight + ', New: ' + newHeight
				);
			}
			...
			if (this._isInitialFillComplete) {
				...
				for (
					let i = this._itemsAboveRenderBlock;
					i < this._itemsAboveRenderBlock + this._itemsInRenderBlock;
					i++
				) {
				...
				if (!foundVisibleItemBefore) {
					...
					if (this.props.logInfo) {
						this.props.logInfo(
							'Added delta to fake space offset: ' +
								(oldHeight - newHeight) +
								' -> ' +
	...
	private _calcNewRenderedItemState(props: VirtualListViewProps<ItemInfo>): void {
		...
		while (this._itemsInRenderBlock > 0) {
			...
			const itemHeight = this._getHeightOfItem(item);
			if (
				this._heightAboveRenderAdjustment +
					this._heightAboveRenderBlock +
					this._heightOfRenderBlock -
					itemHeight <=
				bottomCullLine
			) {
				break;
		...
		if (this._itemsInRenderBlock === 0) {
			...
			this._itemsBelowRenderBlock =
				props.itemList.length - this._itemsAboveRenderBlock - this._itemsInRenderBlock;
			this._heightAboveRenderBlock = this._calcHeightOfItems(props, 0, this._itemsAboveRenderBlock - 1);
			this._heightOfRenderBlock = this._calcHeightOfItems(
				props,
				this._itemsAboveRenderBlock,
		...
		while (this._pendingMeasurements.size < _maxSimultaneousMeasures) {
			...
			if (
				...
				this._heightAboveRenderAdjustment + this._heightAboveRenderBlock + this._heightOfRenderBlock >=
					renderBlockBottomLimit
			) {
				break;
			}
		...
		if (Math.abs(this._heightAboveRenderAdjustment) > maxFakeSpaceOffset) {
			...
			for (let i = this._itemsAboveRenderBlock; i < this._itemsAboveRenderBlock + this._itemsInRenderBlock; i++) {
				...
				this._setCellTopAndVisibility(
					item.key,
					cell.isVisible,
					cell.top - this._heightAboveRenderAdjustment,
					false
				);
			}
	...
	private _resizeAllItems(_props: VirtualListViewProps<ItemInfo>) {
	...
	private _renderIfDirty(_props: VirtualListViewProps<ItemInfo>): void {
	...
	private _allocateCell(
		...
		if (!newCell) {
			...
			if (itemTemplate && isHeightConstant) {
				...
				let bestOptionIndex = _.findIndex(
					this._recycledCells,
					cell =>
						cell.itemTemplate === itemTemplate && cell.cachedItemKey === itemKey && cell.height === height
				);
	...
	render() {
		...
		for (const cell of cellList) {
			...
			itemsRendered.push(
				<VirtualListCell
					...
					key={
						this._isAndroidScreenReaderEnabled()
							? _accessibilityVirtualKeyPrefix + cell.cellInfo.virtualKey
							: cell.cellInfo.virtualKey
					}
		...
		let staticContainerStyle: (RX.Types.ViewStyleRuleSet | RX.Types.AnimatedViewStyleRuleSet)[] = [
			_styles.staticContainer,
		];
		if (this.props.style) {
		...
		return (
			...
			>
				<RX.Animated.View style={staticContainerStyle}>{itemsRendered}</RX.Animated.View>
			</RX.ScrollView>
		);
	}
	...
	private _onKeyDown = (e: RX.Types.KeyboardEvent) => {
		if (!this._scrollViewRef.current || (e.keyCode !== _keyCodeUpArrow && e.keyCode !== _keyCodeDownArrow)) {
			return;
		}
	...
	private _scrollToItemIndex(index: number): void {
		this.scrollToTop(
			true,
			this._calcHeightOfItems(this.props, 0, index - 1) - (this.props.keyboardFocusScrollOffset || 0)
		);
	}
	...
	private _focusSubsequentItem(direction: FocusDirection, viaKeyboard: boolean, retry = true): boolean {
		let index: number | undefined = _.findIndex(
			this._navigatableItemsRendered,
			item => item.key === this.state.lastFocusedItemKey
		);
	...
	componentDidUpdate(_prevProps: VirtualListViewProps<ItemInfo>) {
		this._componentDidRender();
	}
	...
	private _shouldShowItem(item: VirtualListViewItemInfo, _props: VirtualListViewProps<ItemInfo>): boolean {
