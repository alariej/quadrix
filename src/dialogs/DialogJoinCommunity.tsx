...
import {
	theSearchDidNotReturn,
	theSearchTakesTooLong,
	cancel,
	theSearchReturnedError,
	theCommunity,
	pressOKToJoin,
	search,
	communityName,
...
export default class DialogJoinCommunity extends RX.Component<DialogJoinCommunityProps, RX.Stateless> {
	...
	private searchCommunities = () => {
		...
		const showCancelSearch = () => {
			if (getPublicRoomsCallReturned) {
				return;
			}

			SpinnerUtils.dismissModalSpinner('searchcommunityspinner');

			const text = <RX.Text style={styles.textDialog}>{theSearchTakesTooLong[this.language]}</RX.Text>;
		...
		ApiClient.getPublicRooms(options, this.serverName)
			.then(response => {
				...
				if (!response || !response.chunk || response.chunk.length === 0) {
					...
					const text = <RX.Text style={styles.textDialog}>{theSearchDidNotReturn[this.language]}</RX.Text>;
	...
	private showRoomList = (rooms: PublicRoom_[]) => {
		const roomTiles = rooms.map(room => {
			return (
				<CommunityTile
					key={room.room_id}
					roomResponse={room}
		...
		const roomList = (
			...
			>
				<RX.View
					style={[styles.modalView, { maxHeight: (TILE_HEIGHT_COMMUNITY + 1) * roomTiles.length }]}
	...
	private askJoinCommunity = (roomId: string, roomName: string) => {
		...
		const text = (
			<RX.Text style={styles.textDialog}>
				<RX.Text>{pressOKToJoin[this.language]}</RX.Text>
				<RX.Text style={{ fontWeight: 'bold' }}>{' ' + roomName}</RX.Text>
				<RX.Text>{theCommunity[this.language]}</RX.Text>
			</RX.Text>
		);
	...
	public render(): JSX.Element | null {
		...
		return <RX.View style={styles.modalScreen}>{joinCommunityDialog}</RX.View>;
	}
}
