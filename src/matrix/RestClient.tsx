...
export default class RestClient extends GenericRestClient {
    ...
    public register(data: { auth?: AuthParam_, username: string, password: string, inhibit_login: boolean}): Promise<LoginResponse_> {

        return this.performApiPost<LoginResponse_>('register?kind=user', data);
    }

    public requestEmailToken(data: { client_secret: string, email: string, send_attempt: number}): Promise<EmailTokenResponse_> {
    ...
    public getSyncFiltered(syncToken: string, filter: SyncFilter_, syncTimeout: number, fullState: boolean): Promise<SyncResponse_> {
    ...
    public getRoomMessages(roomId: string, limit: number, dir: string, from?: string, to?: string, filter?: { types: string[] })
